...
export function Jion()  {
    ...
    const unirseAPartida = async () => {
      try {
        ...
        // Actualiza la lista de jugadores inmediatamente
        const jugadoresActuales = await getDocs(jugadoresRef);
        jugadores.value = jugadoresActuales.docs.map((doc) => doc.data());
